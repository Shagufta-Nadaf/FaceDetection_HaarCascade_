...
using namespace dnn;
using namespace std;
// with 320 its not taking far faces so we go with 820...
const int YU_NET_INPUT_SIZE = 820;

int main() {
    // Load the YuNet model from Downloads
    ...
    while (true) {
        ...
        for (int i = 0; i < faces.rows; i++) {
            ...
            cout << "Detected face coordinates (scaled): (" 
                 << x1 << ", " << y1 << ", " 
                 << w << ", " << h << ") with confidence: " 
                 << confidence << endl;
            ...
            // Debugging output for original coordinates
            cout << "Bounding box coordinates (original): (" 
                 << originalX1 << ", " << originalY1 << ", " 
                 << originalX2 << ", " << originalY2 << ")" << endl;
        ...
        imshow("Face Detection", frame); // Show the output frame
